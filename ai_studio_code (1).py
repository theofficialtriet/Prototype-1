...
if ticker_symbol:
    ...
    # Load Market Data (used to calculate WACC and extract standardized DCF vectors)
    yf_ticker, yf_info, yf_financials, yf_balance, yf_cashflow, yf_error = load_market_vars(ticker_symbol)
    ...
    if sec_data.get("error") or yf_error:
        ...
        with tab_is:
            raw_is_df = sec_data["income"].to_dataframe(view=view_mode)
            st.dataframe(clean_sec_dataframe(raw_is_df), use_container_width=True)
            
        with tab_bs:
            raw_bs_df = sec_data["balance"].to_dataframe(view=view_mode)
            st.dataframe(clean_sec_dataframe(raw_bs_df), use_container_width=True)
            
        with tab_cf:
            raw_cf_df = sec_data["cashflow"].to_dataframe(view=view_mode)
            st.dataframe(clean_sec_dataframe(raw_cf_df), use_container_width=True)
