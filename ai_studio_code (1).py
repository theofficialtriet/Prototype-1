...
">
    <span style="color: #71767B; font-size: 0.75rem; letter-spacing: 0.05em; text-transform: uppercase; font-weight: 700; display: block;">Live US 10-Yr Bond Yield (CNBC)</span>
...
if ticker_symbol:
    ...
    # Resolve exact lists under Advanced Mode (Set labels safely to prevent NameError on any non-US or failed tickers)
    if advanced_mode and not sec_failed:
        # Load Raw Statements
        raw_sec_inc = clean_sec_dataframe(sec_data.get("income_standard", None))
        ...
        raw_sec_bal = clean_sec_dataframe(sec_data.get("balance_standard", None))
        ...
        raw_sec_cf = clean_sec_dataframe(sec_data.get("cashflow_standard", None))
        ...
        with tab_cf:
            ...
            if not advanced_mode or sec_failed or len(exact_cf_items) == 0:
                ...
                edited_cf_df = st.data_editor(
                    ...
                    use_container_width=True,
                    column_config=pct_column_config,
                    key="cf_editor_advanced_v2"
                )

        # Extraction logic with type casting & fallback validation
        if not advanced_mode or sec_failed or len(exact_inc_items) == 0:
            ...
            # Under advanced mode, drivers are extracted dynamically from raw statement rows using exact indexing
