...
# Generates advanced metrics inputs dynamically from the raw filing rows (TypeError resolved using iterrows() Series loop)
def extract_numeric_rows_for_advanced(df):
    ...
    for idx, row in df.iterrows():
        try:
            # Safely attempt to cast elements to numeric to identify active numeric rows
...
if ticker_symbol:
    ...
    with col_note_1:
        # Dynamically populate exact line items corresponding to active projection tab selection
    ...
    with col_note_3:
        note_year = st.selectbox("Select Target Period", proj_cols)
