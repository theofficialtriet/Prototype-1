...
if ticker_symbol:
    ...
    if yf_error:
        ...
        # --- DYNAMIC EXPANDERS FOR DETAILED FORECAST PARAMETERS ---
        st.sidebar.markdown("<h3 style='color: #C5A880; font-size: 1.15rem;'>Segment Drivers</h3>", unsafe_allow_html=True)
        with st.sidebar.expander("Detailed Year-by-Year Setup", expanded=False):
            p_rev_growth = []
            p_ebit_margin = []
            for y in range(1, forecast_years + 1):
                st.markdown(f"<span style='color: #C5A880; font-weight:600;'>Year {y} Settings</span>", unsafe_allow_html=True)
                col_g, col_m = st.columns(2)
                with col_g:
                    g = st.number_input(f"Rev Growth (%)", value=float(hist_rev_growth*100), key=f"g_{y}", step=0.5) / 100
        ...
        with st.sidebar.expander("Global Balance Sheet & Tax Defaults", expanded=False):
            p_tax_rate = st.number_input("Effective Tax Rate (%)", value=float(hist_tax_rate*100), step=0.5) / 100
            p_capex = st.number_input("CapEx as % of Revenue (%)", value=float(hist_capex_pct*100), step=0.5) / 100
            p_da = st.number_input("D&A as % of Revenue (%)", value=float(hist_da_pct*100), step=0.5) / 100
            p_nwc = st.number_input("NWC Change % of Revenue (%)", value=float(hist_nwc_change_pct*100), step=0.1) / 100
        ...
        # --- TIMELINE EXCEL VIEW ---
        st.markdown("<h3>Integrated Financial Projections</h3>", unsafe_allow_html=True)
        ...
        # --- REFERENCE SHEETS ---
        st.markdown("<h3>Filing Verification Worksheets</h3>", unsafe_allow_html=True)
        if not sec_failed:
