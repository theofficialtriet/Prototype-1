...
if ticker_symbol:
    ...
    if sec_data.get("error") or yf_error:
        st.error(f"Error retrieving financial files. Please verify that your User-Agent Email is configured and the ticker exists.")
    else:
        # --- PREPARE HISTORICAL DATA VECTOR ---
        ...
        st.subheader("SEC Filing Reference Sheets")
        st.markdown("Raw source files loaded directly from official SEC databases.")
